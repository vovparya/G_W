...
def process_image(image_feed_id):
    try:
        ...
        # Получаем абсолютный путь к текущей директории (где находится utils.py)
        current_dir = os.path.dirname(os.path.abspath(__file__))

        # Конструируем абсолютные пути к файлам модели
        model_path = os.path.join(current_dir, 'mobilenet_iter_73000.caffemodel')
        config_path = os.path.join(current_dir, 'mobilenet_ssd_deploy.prototxt')
    ...
    # Это позволит отлавливать и выводить другие возможные ошибки, что облегчит отладку.
